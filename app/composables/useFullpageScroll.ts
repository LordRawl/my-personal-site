...
export const useFullpageScroll = () => {
  ...
  const decide = (dir: 1 | -1): number | null => {
    ...
    if (dir === 1) {
      ...
      // Секция считается «высокой» (нужен native-скролл внутри) если её контент
      // превышает область под шапкой (vh - header): низ секции не помещается,
      // и пользователю нужно докрутить колесом, чтобы увидеть всё до конца.
      // Секции, помещающиеся ровно в область, снапятся за один тик.
      const isTall = item.el.offsetHeight > area + 1
